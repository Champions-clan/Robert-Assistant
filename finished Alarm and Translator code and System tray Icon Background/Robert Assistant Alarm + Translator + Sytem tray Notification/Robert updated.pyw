import pyttsx3, speech_recognition as sr, datetime, datefinder, time, wikipedia, webbrowser
...
def take_speak_command():
    ...
    while(True):
        ...
        if "translate" in statement or "change" in statement:
            translator = Translator()

            speak("Which word you want to translate?")
            translate = ("\ntWhich word you want to translate?")
            text = takeCommand()

            speak("Which Language you want to translate?")
            language = ("\ntWhich Language you want to translate?")
            speech = takeCommand()
            
            translation = translator.translate(text,dest=speech)
            print(translation) 
            speak(translation)

        if "alarm" in statement or "reminder" in statement:
            ...
            while(1 == 1):
                if(alarmH == datetime.datetime.now().hour and alarmM == datetime.datetime.now().minute):
                    print(Reminder)
                    notify.show_toast("Notification", "Reminder!!!", duration = 5, icon_path = "C:\\Users\\acer\\Desktop\\facerecong\\Robert_Virtual_Assistant-master\\Robert_Virtual_Assistant-master\\Asserts\\logo.ico") and playsound("C:\\Users\\acer\\Desktop\\facerecong\\Robert_Virtual_Assistant-master\\Robert_Virtual_Assistant-master\\Asserts\\beep.wav")
                    break
